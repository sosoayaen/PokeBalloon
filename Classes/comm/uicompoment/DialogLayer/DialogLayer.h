...
// 使用：
//       1.需要响应事件的按钮使用pushMenu添加
//       2.派生类需要实现宏CREATE_FUNC(class_name)
...
public:
	...
	*/
	...
	/**
	* @brief 点击在面板区域外时触发的函数
	*		 默认是调用结束函数，子类重载即可修改此行为
	*/
	virtual void onClickOutSideOfVisibleBoard();
	...
	* @brief 设置进入动画
	*/
	void setOnEnterAction(cocos2d::CCActionInterval* pOnEnterAction);

	/**
	* @brief 设置退出动画
	*/
	void setOnExitAction(cocos2d::CCActionInterval* pOnExitAction);

	/**
	* @brief 设置关闭对话框的回调函数
	*/
	void setEndCallbackFuncN(cocos2d::CCCallFuncN* pCallbackFuncN);

	/**
	* @brief 点击屏幕任何位置都关闭开关，默认不启用。一旦启用，则忽略内部的按钮响应
	* @param bEnable 是否启用的标志
	*/
	void setCloseDialogClickAnyWhere(bool bEnable);

    bool  isNodeVisible(cocos2d::CCNode* pNode);

	/**
	* @brief 优先设置判断点击的区域
	* @param rect 这里需要提供屏幕坐标下的rect!!!
	*/
	void setClickValideRect(const cocos2d::CCRect& rect);
...
//    bool mTouchedMenu;                  // 记录菜单点击
	...
	*/
	...
	// 点击屏幕任何位置关闭对话框的标志
	bool m_bClickAnyWhereCloseDialog;
private:
	// 整体面板进场动画
