...
//生成密文
void CMD5Encrypt::EncryptData(char* pszSrcData, char szMD5Result[16])
{
	if (pszSrcData == NULL || pszSrcData == NULL)
	{
		//ASSERT(false);
		return ;
	}
...
//生成密文
unsigned short CXOREncrypt::EncryptData(char* pszSrcData, char* pszEncrypData, unsigned short wSize)
{
...
//	
	...
	int nLen = CountArray(szRandKey);
	for (unsigned short i=1;i<nLen;i++)
	{
...
//解开密文
unsigned short CXOREncrypt::CrevasseData(char* pszEncrypData, char* pszSrcData, unsigned short wSize)
{
	//效验参数
	//ASSERT(wSize>0);
	//ASSERT(pszSrcData!=NULL);
	...
	if (pszEncrypData == NULL || pszSrcData == NULL)
	{
		//ASSERT(false);
		return -1;
	}
